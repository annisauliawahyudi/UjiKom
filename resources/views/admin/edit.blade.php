<div id="authentication-modal-edit-{{ $user->id }}" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
            ...
            <div
                ...
                <button type="button"
                    ...
                    data-modal-hide="authentication-modal-edit-{{ $user->id }}">
            ...
            <div class="p-4 md:p-5">
                <form class="space-y-4" action="{{ route('admin.user.update', $user->id) }}" method="POST">
                    ...
                    <div>
                        ...
                        <select id="provinsi-{{ $user->id }}" name="provinsi"
...
<script>
    function fetchProvinsi(userId, selectedValue = null) {
        fetch(`https://kanglerian.my.id/api-wilayah-indonesia/api/provinces.json`)
            .then(response => response.json())
            .then(provinces => {
                var tampung = '<option>Pilih Provinsi</option>';
                provinces.forEach(element => {
                    let selected = selectedValue === element.name ? 'selected' : '';
                    tampung += `<option data-reg="${element.id}" value="${element.name}" ${selected}>${element.name}</option>`;
                });
                document.getElementById('provinsi-' + userId).innerHTML = tampung;
            });
    }
</script>
